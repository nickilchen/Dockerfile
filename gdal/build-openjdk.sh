...
# 配置变量
REGISTRY=${REGISTRY:-""}
IMAGE_NAME=${IMAGE_NAME:-"gdal-openjdk"}
TAG=${TAG:-"3.7.1-openjdk8u342"}
...
check_dependencies() {
    ...
    # 检查Dockerfile
    if [ ! -f "Dockerfile_openjdk" ]; then
        log_error "当前目录下没有找到Dockerfile_openjdk"
        exit 1
    fi
...
setup_builder() {
    ...
    BUILDER_NAME="gdal-openjdk-builder"
...
# 构建镜像
build_image() {
    log_info "开始构建GDAL OpenJDK多架构镜像..."
    
    # 构建命令参数
    BUILD_CMD="docker buildx build"
    BUILD_CMD="$BUILD_CMD --platform $PLATFORMS"
    BUILD_CMD="$BUILD_CMD --tag ${FULL_IMAGE_NAME}:${TAG}"
    BUILD_CMD="$BUILD_CMD -f Dockerfile_openjdk"
...
选项:
    ...
    -n, --name NAME            镜像名称 (默认: gdal-openjdk)
    -t, --tag TAG              镜像标签 (默认: 3.7.1-openjdk8u342)
...
main() {
    ...
    log_success "GDAL OpenJDK多架构镜像构建完成！"
    log_info "镜像名称: ${FULL_IMAGE_NAME}:${TAG}"
    log_info "支持平台: ${PLATFORMS}"
