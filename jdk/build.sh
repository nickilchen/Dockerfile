...
# 配置变量
REGISTRY=${REGISTRY:-""}
IMAGE_NAME=${IMAGE_NAME:-"minimal-jdk8"}
TAG=${TAG:-"8u431-alpine"}
...
check_dependencies() {
    ...
    # 检查JDK包
    if [ ! -f "jdk-8u431-linux-x64.tar.gz" ]; then
        log_error "未找到jdk-8u431-linux-x64.tar.gz文件"
        exit 1
    fi
...
选项:
    ...
    -n, --name NAME            镜像名称 (默认: minimal-jdk8)
    -t, --tag TAG              镜像标签 (默认: 8u431-alpine)
    -p, --platforms PLATFORMS  支持平台 (默认: linux/amd64)
    --push                     构建后推送到仓库
    --no-cache                 不使用缓存构建
    -h, --help                 显示帮助信息
...
main() {
    ...
    log_success "最小JDK 8镜像构建完成！"
    log_info "镜像名称: ${FULL_IMAGE_NAME}:${TAG}"
    log_info "镜像大小已优化以达到最小空间占用"
    
    echo
    log_info "可以使用以下命令运行容器:"
    echo "docker run -it --rm ${FULL_IMAGE_NAME}:${TAG} java -version"
    echo
}
